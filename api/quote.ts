// api/quote.ts
// Disable TS checking for this file so Vercel doesn't complain about Node types.
// @ts-nocheck
...
export default async function handler(req, res) {
  ...
  const forwardEmail = process.env.QUOTE_TO_EMAIL;   // Neighborhoodkrew@gmail.com
  ...
  if (!fromEmail) {
    ...
    return res
      .status(500)
      .json({ ok: false, error: "Sender email not configured." });
  }

  const { name, email, phone, service, details } = req.body || {};

  if (!name || !email || !service || !details) {
    console.error("Missing fields in /api/quote", req.body);
    return res
      .status(400)
...
— Neighborhood Krew Inc
  ...
  try {
    // Internal notification – goes to your Gmail
